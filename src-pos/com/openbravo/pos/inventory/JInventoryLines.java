...
public class JInventoryLines extends javax.swing.JPanel {
    ...
    public JInventoryLines() {
        ...
        c = new TableColumn(0, 80
                , new DataCellRenderer(javax.swing.SwingConstants.LEFT)
                , new DefaultCellEditor(new JTextField()));
        c.setHeaderValue(AppLocal.getIntString("label.codigo"));
        columns.addColumn(c);
        c = new TableColumn(1, 350
                , new DataCellRenderer(javax.swing.SwingConstants.LEFT)
                , new DefaultCellEditor(new JTextField()));
        c.setHeaderValue(AppLocal.getIntString("label.item"));
        columns.addColumn(c);
        c = new TableColumn(2, 50
        ...
        c = new TableColumn(3, 90
                , new DataCellRenderer(javax.swing.SwingConstants.RIGHT)
                , new DefaultCellEditor(new JTextField()));
        c.setHeaderValue(AppLocal.getIntString("label.presentacion"));
        columns.addColumn(c);
        c = new TableColumn(4, 50
                , new DataCellRenderer(javax.swing.SwingConstants.RIGHT)
                , new DefaultCellEditor(new JTextField()));
        c.setHeaderValue(AppLocal.getIntString("label.piezas"));
        columns.addColumn(c);
        
        m_tableinventory.setColumnModel(columns);       
        
        m_tableinventory.getTableHeader().setReorderingAllowed(false);         
        m_tableinventory.setRowHeight(25);
        m_tableinventory.setFont(new Font("Arial",1,14));
    ...
    private static class InventoryTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int row, int column) {
            ...
            switch (column) {
                case 0: return i.getProductReference();
                case 1: return i.getProductName();
                case 2: return "x" + Formats.DOUBLE.formatValue(i.getMultiply());
                case 3: return i.getNombrePresentacion();
                case 4: return Formats.INT.formatValue(i.getMultiply()*i.getUnidades());
                default: return null;
            }

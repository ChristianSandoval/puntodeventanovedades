    ...
    private void readTicket(int iTicketid, int iTickettype) {
        ...
        try {
            TicketInfo ticket = (iTicketid==-1) 
                ? m_dlSales.loadTicket(iTickettype,  m_jTicketEditor.getValueInteger(), m_App.getInventoryLocation())
                : m_dlSales.loadTicket(iTickettype, iTicketid, m_App.getInventoryLocation()) ;
        ...
        if (m_ticket != null) {
            try {
                ...
                if (m_App.getInventoryLocation().equals("0")) {
                                    printReport("/com/openbravo/reports/ticketsample", m_ticket, null);
                                }
                else

...
public class JCatalog extends JPanel implements ListSelectionListener, CatalogSelector {
    ...
    public JCatalog(DataLogicSales dlSales) {
        this(dlSales, false, false, 300, 32);
    }
    ...
    private String getProductLabel(ProductInfoExt product) {
        ...
        if (pricevisible) {
            if (taxesincluded) {
                ...
                return "<html><center>" + product.getReference()+"-"+product.getName() + "<br>" + product.printPriceSellTax(tax);
            } else {
                return "<html><center>" + product.getReference()+"-"+product.getName() + "<br>" + product.printPriceSell();
            }
        } else {
